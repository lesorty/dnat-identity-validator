...
\texttt{dnat-executor} (CVM3)---and is contrasted with two baselines: a
single-container deployment that colocates the same assets (A) and a no-isolation
variant that removes the microVM entirely (M0). Each plane is a container in the current
prototype, standing in for a hardware-backed confidential
VM~\cite{amd2020sevsnp} in a production deployment; because the planes interact
only through their declared interfaces and isolation boundaries, this
substitution changes the execution substrate but not the architecture or the
security argument evaluated in Section~\ref{sec:evaluation}. We therefore do not
report confidential-computing overheads, and the analysis that follows holds
under a fully confidential deployment.
...
dataset and the application. A purchased right is recorded under the key
\texttt{keccak256(encryptedDatasetHash, encryptedApplicationHash, buyer)}, bound
to the encrypted IPFS CIDs of the two assets and the buyer's address rather than
to mutable asset identifiers, so the grant is stable under off-chain re-indexing
and is verified before any bundle is assembled.
...
Figure~\ref{fig:exec-lifecycle} summarizes this per-request lifecycle.

\begin{figure}[htbp]
\footnotesize
\begin{verbatim}
execute(bundle):                      # one /execute request [host]
  workdir  = fresh per-run directory
  rootfs   = copy-on-write overlay of the base image
  in_disk  = new ext4, read-only, holding the bundle   # INPUT role
  out_disk = new ext4, writable, tagged by a marker    # OUTPUT role
  vm = Firecracker(rootfs, [in_disk, out_disk, app_disk?],
                   vcpu=1, mem=512 MiB, NO network)     # no eth0
  vm.start()
  wait_serial(vm, "EXECUTION_COMPLETE")   # guest signals completion
  vm.shutdown(graceful, then kill on timeout)
  result = read(out_disk)                 # {rc, stdout, stderr}
  on exit: umount loop devices; rm -rf workdir          # ephemeral

guest (disk roles are discovered, not passed in):
  find disks by marker; extract the bundle into /tmp/exec
  run workspace/run.sh   (--dataset PATH, else positional)
  write {rc, stdout, stderr} to OUTPUT; print EXECUTION_COMPLETE
\end{verbatim}
\caption{Per-request lifecycle of the execution plane (CVM3): a fresh,
networkless Firecracker microVM per run, disk roles discovered by marker rather
than passed in out of band, completion signaled over the serial console, and a
cleanup trap that leaves no residual state.}
\label{fig:exec-lifecycle}
\end{figure}

\subsection{Application Contract and Dependency Reuse}

An application is registered as unmodified source together with a manifest that
declares its third-party dependencies. The builder (CVM2) resolves those
dependencies and bundles them, with the source, into the read-only
\texttt{application.ext4} artifact, and the executor (CVM3) runs the application
through the fixed \texttt{workspace/run.sh} entry point, passing the dataset by a
small set of conventions (a \texttt{--dataset} flag or a positional path). Because
dependencies are shipped as ordinary \texttt{site-packages} inside the artifact---
rather than linked into a trusted-execution enclave at build time---an application
that follows this simple contract runs with arbitrary declared dependencies, and
runs unchanged across all three environments, without recompilation or
enclave-specific adaptation. This is the reuse property that
Section~\ref{sec:evaluation} contrasts with the enclave-based original DNAT, where
third-party dependencies require recompilation or adaptation.

\subsection{Asset Integrity and Baselines}
...
contract. The first baseline
(\texttt{docker/baseline-vm.compose.yaml}, environment~A) runs the identical control, build, and
...
A second baseline removes isolation altogether. With \texttt{DNAT\_NO\_MICROVM}
set, the executor runs \texttt{vm/run-direct.sh}, which executes the identical
\texttt{workspace/run.sh} contract directly in the container's namespace---no
Firecracker, no per-run input and output disks, and no network removal---while
emitting the same \texttt{\{returncode, stdout, stderr\}} result so the request
path is otherwise unchanged. This no-isolation variant (M0) is the lower endpoint
of the spectrum in Section~\ref{sec:evaluation}: the application inherits the
container's network, environment (including the control-plane secrets), and
persistent filesystem, which lets the evaluation separate the microVM's
operational cost from the containment it provides.
