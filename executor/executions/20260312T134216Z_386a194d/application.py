...
try:
    ...
    raise SystemExit(
        ...
        "Run this script with the project virtualenv, for example "
        "'.venv\\Scripts\\python.exe'."
    ) from exc


def resolve_certs_dir() -> Path:
    env_value = Path.cwd()
    for base_dir in [Path(__file__).resolve().parent, *Path(__file__).resolve().parents]:
        candidate = base_dir / "assets" / "certs" / "icp-brasil"
        if candidate.is_dir():
            return candidate
        candidate = base_dir / "certs" / "icp-brasil"
        if candidate.is_dir():
            return candidate
    candidate = env_value / "assets" / "certs" / "icp-brasil"
    if candidate.is_dir():
        return candidate
    raise FileNotFoundError(
        "Could not locate 'assets/certs/icp-brasil'. "
        "Run from the repository root or set up the execution environment with the cert bundle."
    )
